...
public class LinkedList<E> implements List<E> {
    ...
    private class LinkedListIterator extends LinkedIterator{
        ...
        //constructs with the supers constructor
        public LinkedList LinkedListIterator(){
            return new LinkedList();
        }

        //PotsCondition: returns the next E in the list
        public E next(){
            forward=true;
            return super.next();
        }

        //Returns boolean telling if there is an next node in the list
        public boolean hasNext(){
            return super.hasNext();
        }

        //Returns: boolean telling if there is a previous node in the list
        public boolean hasPrevious(){
            return removeOK;
        }

        //Returns: the previous node in the list, throws illegalStateException
        ...
        //Precondition: passed a node in the list
        //Returns: an int index 
        public int getIndex(ListNode<E>node){
            int index =-1;
            int count=0;
            while(node.next!=null){
                if(node.next==node&&count>index){
                    index=count;
                }
                count++;
                node=node.next;
        ...
        //Returns: int representing the index of the next node
        ...
        //Returns: int representing the previous node
        ...
        //Postcondition: removes the current node from the list
        public void remove(){
            if(removeOK!=true){
                throw new IllegalStateException();
            }
        ...
        //Precondition: passes a E and there is at least on Node in the list
        //Postcondition:depending on if we went prev or next last sets current 
        //              to the passed in value
        public void set(E value){
            if(removeOK==false){
                throw new IllegalStateException();
            }
            ListNode<E> old = (forward) ? current.prev : current.next;
            old.data = value;
        }
        
        //Precondition: passed a E 
        //Postcondition: passed in E is added to the list
        public void add(E value){
           /* ListNode<E> node = new ListNode<E>(value);
            ListNode<E> temp = new ListNode<E>(current.next);
            current.next=node;
